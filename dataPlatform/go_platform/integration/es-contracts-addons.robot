...
es-contract - update - add addon with same start_date after remove addon
    [Tags]    CID:5366
    [Setup]    Update Addons To Es Contracts
    Es Contracts Update Addons    ${es_contract_id}    ${es_plan_Id}    2    2    ${es_addons_Id}    ${current_timestamp}    ${future_time}    ${current_timestamp}    1    0
...
es-contract - update - add addon with same start_date twice should be failed
    [Tags]    FET    CID:5367
    [Setup]    Update Addons To Es Contracts
    Es Contracts Update Addons    ${es_contract_id}    ${es_plan_Id}    2    2    ${es_addons_Id}    ${current_timestamp}    ${future_time}    ${current_timestamp}    1    0
    Es Contracts Update Addons    ${es_contract_id}    ${es_plan_Id}    2    1    ${es_addons_Id}    ${current_timestamp}    ${future_time}    ${current_timestamp}    1    0
    ${resp} =    Es Contracts Update Addons    ${es_contract_id}    ${es_plan_Id}    2    1    ${es_addons_Id}    ${current_timestamp}    ${future_time}    ${current_timestamp}    1    0
    Verify Status Code As Expected             ${resp}    200
    Verify GoPlatform Error Message    ${resp}    604040002    ERROR: The addon is already assigned to the ES contract

es-contract - update - remove activate addon
    [Tags]    CID:5365
    [Setup]    Update Addons To Es Contracts
    Es Contracts Update Addons    ${es_contract_id}    ${es_plan_Id}    2    2    ${es_addons_Id}    ${current_timestamp}    ${future_time}    ${current_timestamp}    1    0
    ${resp} =    Es Contracts Addons Get    ${es_contract_id}
...
Get Es Addons Id
    ${resp} =    Es Addons Get
    ...    addon_id=${None}     status=1    published=1
    ...    valid_time_from=0    valid_time_to=${current_timestamp}
...
SuiteSetup
    ...
    ${future_time} =    Evaluate    int(time.time() + 200)    time
