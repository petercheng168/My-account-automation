...
# -------- Gogoro Keywords --------
Create Parts
    [Documentation]    Create Parts Price
    [Arguments]    ${Part}
    ${resp} =    Parts Post    sequence_id=${Part.sequence_id}    part_code=${Part.part_code}    part_type=${Part.part_type}\
    ...    part_sub_type=${Part.part_sub_type}    parts_category_id=${Part.part_category_id}    brand_company_code=${Part.brand_company_code}\
...
Update Part
    ...
    [Arguments]    ${Part}
     ${resp} =    Parts Prices Update   sequence_id=${Part.sequence_id}    part_code=${Part.part_code}    part_type=${Part.part_type}\
    ...    part_sub_type=${Part.part_sub_type}    parts_category_id=${Part.part_category_id}    brand_company_code=${Part.brand_company_code}\
    ...    names=${Part.names}    production_start_time=${Part.production_start_time}    production_end_time=${Part.production_end_time}\    
    ...    sale_status=${Part.sale_status}    sale_suspend_time=${Part.sale_suspend_time}    warranty_period=${Part.warranty_period}\
    ...    warranty_distance=${Part.warranty_distance}    endurance_period=${Part.endurance_period}    endurance_distance=${Part.endurance_distance}\
