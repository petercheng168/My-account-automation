...
*** Test Cases ***
Disable 30 days free trial sport mode addon
    ...
    Verify Addon Card Message Display As Expected    該服務將於 ${addon_end_date} 停止
    Verify User Addon Code Is Correct    ${es_contract_id}    ${gogoro-sess}    ${csrf_token}    SUBFEE-20002

Enable 30 days free trial sport mode addon
    [Setup]    Activate Subscription    False
    Go To My-Plan Page
    Click 30 Days Free trial Sport Mode Addon Button
    Click Button Type Button    確認
    Confirm Enable 30 Days Free Addon Dialog
    Verify Addon Card Message Display As Expected    您已開始試用性能提升
    Verify User Addon Code Is Correct    ${es_contract_id}    ${gogoro-sess}    ${csrf_token}    SUBFEE-20002
...
SuiteSetup
    ...
    Log     ${addon_end_date}
    # ${addon_end_date} =    Get Current Date    UTC    result_format=%Y-%m-%d23H:59M:59S.000000Z
...
Verify User Addon Code Is Correct
    ...
    Verify Response Contains Expected    ${resp.json()['items'][1]['sku']}    ${addon_code}
