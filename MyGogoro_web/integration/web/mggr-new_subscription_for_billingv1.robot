...
Create new subscription with addon          #M04-01-01,   M04-01-49,   M04-01-68,  M04-01-74,  M04-01-76
    Click Button Type Button    建立服務合約   #v1.0 建立服務合約, v2.0 為你的新車建立電池服務合約 
    ...
    Upload User Scooter Information For BillingV1    ${Scooter.plate}    ${User.last_name}${User.first_name}    ${Scooter.scooter_vin}    ${last_six_sn}
...
Signup User
    Setup User Variables
    ${resp} =    Users Post    ${User.company_code}    ${User.first_name}    ${User.gender}    ${User.emailv1}    ${User.status}\
    ...
    log to console  login_phone=${User.phone}
    log     ${resp.text}
    Users Update Email Verified          ${user_Id}    1
    # Users Update Login Phone Verified    ${user_Id}    1

Suite Setup
    Open Browser    ${NEWMYGOGORO_PA_DOMAIN}    ${WINDOW_HEIGHT}    ${WINDOW_WEIGHT}
    Signup User
    Login With Email And Password For AC    ${User.emailv1}    Gogoro123
    ${gogoro-sess}    ${csrf_token} =    Sign In MyGogoro Account To Return New Cookie    ${User.emailv1}    ${User.encode_password}
