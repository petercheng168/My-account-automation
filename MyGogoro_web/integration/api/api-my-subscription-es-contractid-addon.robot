*** Settings ***
Documentation    Test suite for get MyGogoro subscription addon
Resource    ../../init.robot
...
*** Test Case ***
Get user subscription none effective addon
    [Setup]    TestSetup    True
    ${resp} =    Api My Subscription Es ContractId Addon Get    ${es_contract_Id}    ${gogoro-sess}    ${csrf_token}
    Verify Status Code As Expected    ${resp}   200
    Verify Schema    ../../..MyGogoro_web/res/schema/api-my-subscription-es-contractId-addon.json    getSubscriptionAddon    ${resp.json()}
    Verify Response Contains Expected    ${resp.json()}    {'total': 2, 'items': [{'id': '${addon_id}', 'sku': 'SUBFEE-20002', 'charge': 249, 'startTime': ${plan_start}}]}

Get without addon user subscription
    [Setup]    TestSetup    False
    ${resp} =    Api My Subscription Es ContractId Addon Get    ${es_contract_Id}    ${gogoro-sess}    ${csrf_token}
    Verify Status Code As Expected    ${resp}   200
    Verify Schema    ../../..MyGogoro_web/res/schema/api-my-subscription-es-contractId-addon.json    getWithoutAddonSubscription    ${resp.json()}
    Verify Response Contains Expected    ${resp.json()}    {'total': 0, 'items': []}
