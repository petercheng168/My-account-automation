...
*** Test Case ***
Get user scooter subscription
    ${resp} =    Api My Scooter ScooterId Subscription Get    ${scooter_Id}    ${gogoro-sess}    ${csrf_token}
    Verify Status Code As Expected    ${resp}   200
    Verify Schema    ../../..MyGogoro_web/res/schema/api-my-scooter-scooterId-subscription.json    getScooterSubscription    ${resp.json()}
    Verify Response Contains Expected    ${resp.json()['id']}    ${es_contract_Id}
    Verify Response Contains Expected    ${resp.json()['status']}    active
    Verify Response Contains Expected    ${resp.json()['plan']}    {'id': '${plan_Id}', 'name': '${plan_name}', 'sku': '${sku}', 'lockedLength': 36, 'chargeType': '${charge_type}', 'chargeBase': ${charge_base}, 'chargeUnit': '${charge_unit}', 'goRewards': 0, 'isIncludesFreeMaintenance': False, 'startTime': ${plan_start}, 'effectiveTime': ${plan_effective_date}, 'endTime': ${plan_end}}
...
TestSetup
    ...
    Set Test Variable    ${plan_name}    ${plan['name']}
